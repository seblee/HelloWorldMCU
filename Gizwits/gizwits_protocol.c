...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flaglegacy)
    {
        info->event[info->num] = EVENT_LEGACY;
        info->num++;
        dataPoints->valuelegacy = gizDecompressionValue(LEGACY_BYTEOFFSET,LEGACY_BITOFFSET,LEGACY_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuelegacy != cur->valuelegacy)
    {
